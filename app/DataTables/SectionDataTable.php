...
class SectionDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function($record) {
                ...
                $action .= '<a class="btn btn-primary btn-sm" title="'. trans("cruds.global.view") .'" href="'.route('admin.parent-section.show', $record->id).'">
                                <i class="fas fa-eye"></i>
                            </a>';
                $action .= '<a class="btn btn-info btn-sm" title="'. trans("cruds.global.edit") .'" href="'.route("admin.parent-section.edit", $record->id).'">
                    <i class="fas fa-pencil-alt"></i>
                </a>';

                $action .= '<form action="'.route('admin.parent-section.destroy', $record->id).'" method="POST" class="deleteForm">
    ...
    {
        return $model->where("level",config("constant.sectionLevel.level1"))->newQuery();
    }
    ...
    {   
        return $this->builder()
        ->setTableId('parent-section-table')
