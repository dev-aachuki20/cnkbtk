...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>{{trans("cruds.blacklist_tag.title_singular")}}</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">{{trans("global.dashboard")}}</a></li>
            <li class="breadcrumb-item active">{{trans("cruds.blacklist_tag.title")}}</li>
          </ol>
        </div>
        ...
        <div class="col-12">
          ...
          <div class="card">
            <div class="card-header">
              <!-- <h3 class="card-title">{{trans("cruds.tag_management.title_singular")}}</h3> -->
              <div class="pull-right">
                <a class="btn btn-success" href="{{route('admin.blacklist-tag.create')}}" style="float:right">{{trans("cruds.global.add")}} {{trans("cruds.blacklist_tag.title_singular")}}</a>
              </div>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
      ...
      Swal.fire({
        ...
        .then((result) => {
          ...
          if (!result.isDismissed) {
            $.ajax({
              type: "POST",
              url: "{{route('admin.blacklist_tag.updateStatus')}}",
              data: {
                _token: "{{csrf_token()}}",
                id: id,
                status: status,
