...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>{{trans("cruds.plan.title_singular")}}</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">{{trans("global.dashboard")}}</a></li>
              <li class="breadcrumb-item active">{{trans("cruds.plan.title_singular")}}</li>
            </ol>
          </div>
        ...
        <div class="row">
          <div class="col-12">
            ...
            <div class="card">
                ...
                <h3 class="card-title">{{trans("cruds.plan.title_singular")}}</h3>
                <div class="pull-right">
                    {{-- <a class="btn btn-success" href="{{route('admin.plan.create')}}" style="float:right">{{trans("cruds.global.add")}} {{trans("cruds.plan.title_singular")}}</a> --}}
                </div>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
    ...
    $(document).on('change click', '.user_status', function(event) {
      ...
      .then((result) => {
          if (!result.isDismissed) {
              $.ajax({
                  type: "POST",
                  url: "{{route('admin.plan.updateStatus')}}",
                  data: {
                      _token: "{{csrf_token()}}",
                      id: id,
                      status: status,
