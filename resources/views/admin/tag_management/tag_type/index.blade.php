...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>{{trans("cruds.tag_management.tag_type.title_singular")}}</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">{{trans("cruds.tag_management.tag_type.title_singular")}}</li>
            </ol>
          </div>
        ...
        <div class="row">
          <div class="col-12">
            ...
            <div class="card">
                ...
                <h3 class="card-title">{{trans("cruds.tag_management.tag_type.title_singular")}}</h3>
                <div class="pull-right">
                    <a class="btn btn-success" href="{{route('admin.tag-type.create')}}" style="float:right">{{trans("cruds.global.add")}} {{trans("cruds.tag_management.tag_type.title_singular")}}</a>
                </div>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
  ...
  $(document).ready( function(){
    $(document).on('submit', '.deleteForm', function(e){
        ...
        swal.fire({
            ...
            text: "{{trans('cruds.tag_management.tag_type.delete_message')}}",
            icon: 'question',
            type: "warning",
            showCancelButton: !0,
    ...
    $(document).on('change click', '.user_status', function(event) {
      ...
      .then((result) => {
          ...
          if (!result.isDismissed) {
              $.ajax({
                  type: "POST",
                  url: "{{route('admin.tag-management.tag-type.updateStatus')}}",
                  data: {
                      _token: "{{csrf_token()}}",
                      id: id,
                      status: status,
