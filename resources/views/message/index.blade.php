    ...
    <div class="container">
        <div class="px-0 chatboxGroup open" id="chatboxGroup">
            <div class="row h-100 gx-3">
                <div class="col-xxl-12 col-lg-12 col-md-12 chat-panel h-100 ">
...
<script>
    ...
    //             // Assuming response.messages contains the messages data
    //             var messages = response.messages;

    //             // Iterate through the messages and generate HTML dynamically
    //             messages.forEach(function(message) {
    //                 var messageHtml = '<div class="message ' + message.type + '">' +
    //                     '<div class="message-content">' + message.content + ' <span class="message_time">' + message.created_at + '</span></div>' +
    //                     '</div>';
    ...
    $('#messageForm').submit(function(e) {
        ...
        if (message !== '') {
            $.ajax({
                ...
                success: function(response) {
                    $('#messageInput').val('');
                    $('#messageContainer').append('<div class="message outgoing">' + message + '</div>');
                },
                error: function(xhr, status, error) {
                    console.error(error);
                }
